    ...
    <div className="products-page">
      ...
      <div className="products-container">
        <aside className={`filters-sidebar ${showFilters ? 'show' : ''}`}>
          ...
          <div className="filter-section"><h3>Buscar</h3><input type="text" placeholder="Buscar produtos..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="search-input" /></div>
          ...
          <div className="filter-section"><h3>Ordenar por</h3><select value={sortBy} onChange={(e) => setSortBy(e.target.value)} className="sort-select"><option value="default">Padrão</option><option value="price-asc">Menor preço</option><option value="price-desc">Maior preço</option><option value="name">Nome (A-Z)</option></select></div>

          <div className="filters-footer"><div className="products-count-sidebar"><p>{filteredAndSortedProducts.length} produtos encontrados</p></div><button onClick={() => setShowFilters(false)} className="apply-filters-btn">Filtrar</button></div>
        </aside>
        ...
        <main className="products-main">
          ...
          {loading ? (
            <div className="no-products"><p>Carregando produtos...</p></div>
          ) : error ? (
            <div className="no-products"><p>⚠️ Erro ao carregar produtos: {error}</p></div>
